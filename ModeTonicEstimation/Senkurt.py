...
class Senkurt:
	"""-------------------------------------------------------------------------
	This is an implementation of Sen(turk)&(Kara)kurt method!
	-------------------------------------------------------------------------"""
	...
	def estimate(self, pitch_file, mode_names=[], mode_name='', mode_dir='./', est_mode=True,
		...
		for idx, pt in enumerate(pts):
			neighbors[idx] = self.chunk_estimate(pt, mode_names=mode_names,
			                                	 mode_name=mode_name, mode_dir=mode_dir, est_tonic=est_tonic, 
				                            	 est_mode=est_mode, distance_method=distance_method,
				                            	 metric=metric, ref_freq=tonic_freq, min_cnt=k_param,
	...
	def chunk_estimate(self, pitch_track, mode_names=[], mode_name='', mode_dir='./',
		...
		tonic_list = [0 for _ in xrange(min_cnt)]
		mode_list = ['' for _ in xrange(min_cnt)]
		...
		#Joint Estimation
		### TODO: The first steps of joint estimation are very similar for both Bozkurt, Senkurt and
